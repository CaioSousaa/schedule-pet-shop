...
async def create_schedule_routes(service_id: int, client_id: UUID, date_schedule: date):
    try:
        create_schedule(
            service_id=service_id, client_id=client_id, date_schedule=date_schedule
        )
        return {"msg": "Agendamento criado com sucesso"}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Erro ao criar o agendamento: {str(e)}"
        )
...
async def get_all():
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao obter os agendamentos: {str(e)}"
        )
...
async def del_schedule(schedule_id: UUID):
    try:
        ...
        raise HTTPException(
            status_code=500, datail=f"Erro ao excluir o agendamento: {str(e)}"
        )
